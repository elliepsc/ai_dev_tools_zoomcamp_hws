...
PostgreSQL is the deployment database (Docker Compose, Kubernetes, CI).  SQLite
remains available as a zero-dependency fallback for quick local hacking.

This module is the only place that knows about backend-specific connection
details and transaction semantics:

* PostgreSQL: a normal READ COMMITTED transaction.  Concurrency is handled by
  row locks taken in :mod:`storage` (``SELECT ... FOR UPDATE [SKIP LOCKED]``),
  always locking the *task* row first so lock ordering is consistent.
* SQLite: ``BEGIN IMMEDIATE`` serializes writers (SQLite has no row locks and
  SQLAlchemy renders ``FOR UPDATE`` as nothing on SQLite).
"""
...
def normalize_database_url(url: str) -> str:
    """Accept the usual ``postgres://`` / ``postgresql://`` forms and pin the
    psycopg 3 driver (the only PostgreSQL driver in the dependencies)."""
...
# Arbitrary constant used as a PostgreSQL advisory-lock key so that several
# replicas starting at once do not race on CREATE TABLE.
_SCHEMA_LOCK_KEY = 7242026


def init_db() -> None:
    """Create the schema, waiting for the database to accept connections.

    In Compose/Kubernetes the API container can start before PostgreSQL is
    ready; retrying here avoids a crash loop on a cold start.
    """

    retries = positive_int("RELAY_DB_CONNECT_RETRIES", 30)
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt == retries:
                raise
            LOGGER.warning("database not reachable yet (attempt %d/%d); retrying", attempt, retries)
            time.sleep(1)
...
def immediate_transaction() -> Generator[Session, None, None]:
    """Run one writer transaction that selects and changes work atomically.

    * SQLite: ``BEGIN IMMEDIATE`` takes the database-wide writer reservation,
      serializing claims, recovery and terminal submissions across processes.
    * PostgreSQL: a regular transaction; callers take row locks with
      ``with_for_update`` (``SKIP LOCKED`` for claims) so independent tasks
      proceed in parallel while each task still has one active lease.
    """
...
def recover_expired_in_session(db: Session, now: datetime) -> int:
    ...
    for attempt in expired:
        # Lock the task row first (same order as claim/heartbeat/complete) and
        # skip it if another transaction is already working on it: that
        # transaction will either finish the attempt or a later pass recovers it.
        task = db.scalar(
            select(Task).where(Task.id == attempt.task_id).with_for_update(skip_locked=True)
        )
        if task is None:
            continue
        # Re-read the attempt now that we hold the task lock.
        db.refresh(attempt)
        if attempt.outcome != "processing" or db_time(attempt.lease_expires_at) > now:
            continue
