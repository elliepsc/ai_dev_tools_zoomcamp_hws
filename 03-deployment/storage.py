...
Claim, heartbeat, terminal submission, and recovery each run inside
``immediate_transaction``.  On PostgreSQL the task row is locked first
(``FOR UPDATE``; ``FOR UPDATE SKIP LOCKED`` for claims so concurrent workers
take different tasks instead of queueing behind each other).  On SQLite the
``FOR UPDATE`` clauses compile to nothing and ``BEGIN IMMEDIATE`` serializes.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # On SQLite, BEGIN IMMEDIATE makes check+insert one operation.  On
    # PostgreSQL two requests with the same key can both miss the check; the
    # unique constraint rejects the loser, which then re-reads the winner.
    try:
        return _create_task_once(sender_id, recipient_id, input_text, idempotency_key)
